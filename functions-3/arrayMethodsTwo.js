...
// CODE HERE
const newDiv2Array = numbers.map((num)=>num/2)
console.log(newDiv2Array)
...
const filteredCities = cities.filter((element)=>element[element.length - 1] === 'i')
...
// CODE HERE
fruits.forEach((elem)=>{if(elem.color === 'red'){
  return console.log(`The fruit with index ${fruits.indexOf(elem)} is an apple`)
} else if(elem.color === `orange`){
  return console.log(`The fruit with index ${fruits.indexOf(elem)} is an orange`)
} else {
  return console.log(`The fruit with index ${fruits.indexOf(elem)} is neither apple or orange`)
}
})
...
/* 
    Count the total price of foods that have "meat" tag. Please type the result too.
*/

// CODE HERE
const riceTotal = foods.reduce((acc,curr)=>{
  if (foods.tags.includes('meat')){
    acc+=curr.price
  }
})
